...
def get_insights(predictions_df):
    """
    Generate business insights and recommendations from predictions.
    ...
    is_increasing = predictions.iloc[-1] > predictions.iloc[0]
    trend_direction = 'increasing' if is_increasing else 'decreasing'
    trend_pct = float(((predictions.iloc[-1] - predictions.iloc[0]) / predictions.iloc[0]) * 100) if predictions.iloc[0] != 0 else 0

    # Peak / low period identification
    peak_idx = predictions.idxmax()
    low_idx = predictions.idxmin()
    peak_date = predictions_df.loc[peak_idx, 'Date']
    low_date = predictions_df.loc[low_idx, 'Date']
    peak_value = float(predictions.max())
    low_value = float(predictions.min())

    # Recommendation sentences
    recommendations = [
        {
            'title': 'Boost Inventory for Peak Times',
            'detail': f"Peak forecast occurs on {peak_date.strftime('%Y-%m-%d')} with projected sales of ${peak_value:,.2f}. Increase stock and staffing around this period."
        },
        {
            'title': 'Optimize Stock for Low Demand',
            'detail': f"Lowest forecast occurs on {low_date.strftime('%Y-%m-%d')} with projected sales of ${low_value:,.2f}. Reduce safety stock and adjust procurement to avoid overstocking."
        },
        {
            'title': 'Run Promotions in High-Demand Periods',
            'detail': f"Current trend is {trend_direction} ({trend_pct:.2f}%). Focus marketing campaigns around rising demand and transit inventory to best-selling categories."
        }
    ]

    insights = {
        'average_forecast': float(predictions.mean()),
        'max_forecast': peak_value,
        'min_forecast': low_value,
        ...
        'peak_month': predictions_df.loc[peak_idx, 'Date'].strftime('%B'),
    }
